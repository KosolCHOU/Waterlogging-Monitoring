...
console.log(
  "%cCropXcel dashboard.js v3 (emoji+why)",
...
if (OVERLAY_URL && OVERLAY_URL.endsWith(".png")) {
  overlayLayer = L.imageOverlay(OVERLAY_URL, BOUNDS, {
    opacity: 0.75,
    pane: "overlayPane",
  }).addTo(map);
}
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Shift" && !shiftHeld) {
    shiftHeld = true;
    if (overlayLayer && overlayVisible) {
      overlayLayer.setOpacity(0.0);
      showToast("Overlay: OFF (Shift)");
    }
  }
});
...
const REASON_FALLBACK = {
  "radar drop (water)": {
    emoji: "💧",
    explain: "Dark radar patch = standing water in field",
  },
  "vh/vv ratio change": {
    emoji: "🌱",
    explain: "Soil/water signal shift = possible saturation",
  },
  "signal variability": {
    emoji: "⚡",
    explain: "Unstable radar = uneven wet/dry spots",
  },
  "water detected": {
    emoji: "💧",
    explain: "Standing water detected in field",
  },
  "high moisture": {
    emoji: "🌊",
    explain: "Soil moisture levels are elevated",
  },
  "flooding risk": {
    emoji: "🚨",
    explain: "High risk of waterlogging detected",
  },
...
const colorFor = (v) =>
  v >= 0.7 ? "#e31a1c" : v >= 0.4 ? "#f59e0b" : "#2ecc71";
function tipsFor(level) {
  if (level === "High")
    return "Standing water likely. Drain within 24h; open outlets/pump.";
  if (level === "Caution")
    return "Watch next 2–3 days. Avoid heavy irrigation; check after rain.";
            ...
            <div style="min-width:320px;max-width:400px;background:#fff;border-radius:16px;
              ...
              <div id="${uidInfo}">
                ...
                <div style="margin-bottom:12px">
                  ...
                  <div style="font-size:13px;color:#0f172a;margin-top:2px">${Number(
                    areaHa
                  ).toFixed(2)} ha</div>
                </div>`
                    : ``
                ...
                <div style="margin-bottom:12px">
                  ...
                  <div style="display:flex;gap:8px;align-items:flex-start;margin-top:4px">
                    ...
                    <div>
                      <div style="font-size:13px;color:#0f172a;"><b>${
                        reason || "Risk detected"
                      }</b></div>
                      ${
                        reasonExplain
                          ? `<div style="font-size:12px;color:#475569;margin-top:2px">${reasonExplain}</div>`
                          : `<div style="font-size:12px;color:#475569;margin-top:2px">Risk analysis indicates potential waterlogging</div>`
                      }
                    </div>
                  </div>
                </div>
...
(async function () {
  try {
    ...
    probeMeta = await fetch(PROBE_META, { cache: "no-store" }).then((r) =>
      r.json()
    );
    const W = probeMeta.cols ?? probeMeta.width;
    const H = probeMeta.rows ?? probeMeta.height;
    probeMeta.width = Number(W);
    probeMeta.height = Number(H);
    probeMeta.scale = probeMeta.scale || 1000;
    if (Array.isArray(probeMeta.web_bounds))
      EFFECTIVE_BOUNDS = probeMeta.web_bounds;

    const buf = await fetch(PROBE_BIN, {
      cache: "no-store",
      method: "GET",
      headers: {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to load probe data: ${response.status} ${response.statusText}`
        );
      }
    ...
    if (buf.byteLength < dataBytes)
      throw new Error("Probe buffer smaller than data_bytes");
    const dataView = buf.slice(0, dataBytes);
    const maskView =
      maskBytes > 0 && buf.byteLength >= dataBytes + maskBytes
        ? buf.slice(dataBytes, dataBytes + maskBytes)
        : null;

    if (dataBytes === npx) {
      probeArray = new Uint8Array(dataView);
      dtype = "u8";
    } else if (dataBytes === npx * 2) {
      probeArray = new Uint16Array(dataView);
      dtype = "u16";
    } else if (dataBytes === npx * 4) {
      probeArray = new Float32Array(dataView);
      dtype = "f32";
    } else {
      probeArray =
        dataBytes % 4 === 0
          ? new Float32Array(dataView)
          : dataBytes % 2 === 0
          ? new Uint16Array(dataView)
          : new Uint8Array(dataView);
      dtype =
        probeArray.BYTES_PER_ELEMENT === 4
          ? "f32"
          : probeArray.BYTES_PER_ELEMENT === 2
          ? "u16"
          : "u8";
    ...
    if (maskView) {
      ...
      if (maskArray.length !== npx) {
        console.warn(
          "Mask size mismatch (ignoring mask)",
          maskArray.length,
          "vs",
          npx
        );
    ...
    console.log("✓ Probe ready:", {
      ...
      w: probeMeta.width,
      h: probeMeta.height,
      scale: probeMeta.scale,
      hasMask: !!maskArray,
    });
  } catch (e) {
    console.error("✗ Probe load failed:", e.message || e);
    canClient = false;
...
function sampleClient(latlng) {
  ...
  if (x < 0 || x >= probeMeta.width || y < 0 || y >= probeMeta.height)
    return null;
  ...
  if (dtype === "u8") v01 = probeArray[idx] / 255;
    ...
    const hasMM =
      Number.isFinite(probeMeta.min) &&
      Number.isFinite(probeMeta.max) &&
      probeMeta.max > probeMeta.min;
...
function loop() {
  ...
  if (!canClient && lastLL && !srvCooldown) {
    srvCooldown = true;
    setTimeout(() => (srvCooldown = false), 200);
...
map.on("click", async (e) => {
  ...
  if (v == null && !canClient) {
    try {
      const r = await fetch(
        `/probe/${JOB_ID}?lat=${e.latlng.lat}&lon=${e.latlng.lng}`
      );
      const d = await r.json();
      if (typeof d?.value === "number") v = clamp01(d.value);
    } catch {}
  }
  if (v == null) return;
  const p = pct(v),
  ...
  const html = `
    <div style="min-width:260px">
      ...
      <div style="margin-top:8px;font-size:12px;color:#64748b"><i>Tip:</i> ${tipsFor(
        lvl
      )}</div>
    </div>`;
  L.popup({ offset: [0, -6] })
...
function sortByColumn(table, colIdx, thEl, dir) {
  ...
  rows.sort((a, b) => {
    ...
    return asc
      ...
      : String(A).localeCompare(String(B)) * -1;
  });
...
function makeSortableAndSparky(t, isTech) {
  ...
  for (let c = 1; c < cols; c++) {
    const sample = rows
      ...
      .map((r) =>
        parseFloat((r.cells[c]?.innerText || "").replace(/[, ]/g, ""))
      );
  ...
  rows.forEach((r) => {
    for (let c = 1; c < cols; c++) {
      ...
      let p = 0;
      if (Number.isFinite(v) && max[c] > min[c])
        p = ((v - min[c]) / (max[c] - min[c])) * 100;
...
function buildDonutMatchBars() {
  ...
  segs.forEach((seg, i) => {
    ...
    const color =
      row.style.getPropertyValue("--c") ||
      getComputedStyle(row).getPropertyValue("--c") ||
      "#9ca3af";
...
function renderTables() {
  ...
  if (window.__techMode__) {
    title.innerHTML =
      'Technical Details <span class="hint" data-tip="Detailed raw indicators from Sentinel-1 (VH/VV, ratios, etc.). Useful for agronomists and advanced users."></span>';
    tableWrap.innerHTML =
      window.__techHTML__ || "<div class='empty'>No records.</div>";
    ...
    title.innerHTML =
      'Per-pass Insights <span class="hint" data-tip="Shows each satellite pass. Farmers can check the date, risk status, and suggested action. Helpful for deciding when to irrigate or drain."></span>';

    // Debug what we're setting
    console.log(
      "🔧 Setting farmer HTML (first 200 chars):",
      window.__farmerHTML__?.substring(0, 200)
    );

    tableWrap.innerHTML =
      window.__farmerHTML__ || "<div class='empty'>No records.</div>";
...
techBtn.onclick = () => {
  window.__techMode__ = !window.__techMode__;
  renderTables();
};
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape") setMapOnly(false);
});
...
async function loadInsights() {
  ...
  try {
    ...
    const res = await fetch(
      `/fields/${FIELD_ID}/insights/?t=${timestamp}&v=${jobId}&_=${Math.random()}`,
      {
        cache: "no-store",
        headers: {
    ...
    console.log(
      "🚨 About to call renderTables() - this will replace the table!"
    );
    renderTables();
  } catch (err) {
    console.error("Insights load failed:", err);
    window.__farmerHTML__ = "";
    window.__techHTML__ = "";
