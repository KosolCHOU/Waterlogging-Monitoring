...
def _risk_palette():
    # NDVI-style: red (high risk) → yellow → green (low risk)
    return mcolors.ListedColormap(
        [
            "#8B0000",  # dark red (high waterlogging risk, low NDVI equivalent)
            "#B22222",  # firebrick
            "#FF4500",  # orange-red
            "#FF6347",  # tomato
            "#FFA500",  # orange
            ...
            "#32CD32",  # lime green
            "#228B22",  # forest green
            "#006400",  # dark green (low waterlogging risk, high NDVI equivalent)
        ]
    )
...
def _save_png01(arr01, out_png):
    rgba = cm.ScalarMappable(
        norm=mcolors.Normalize(0, 1), cmap=_risk_palette()
    ).to_rgba(arr01, bytes=True)
...
def run_analysis_from_notebook(
    ...
    *,
    stack_tif_path: str
    | Path,  # ← path to the exported S1 stack (your notebook export)
    ...
    with rasterio.open(str(tif_path)) as src:
        ...
        arr = {
            i: src.read(i, masked=True).filled(np.nan).astype("float32")
            for i in range(1, min(12, src.count + 1))
        }
    ...
    if 8 in arr:
        drop.append(-arr[8])
    drop01 = (
        _pct_stretch(np.nanmax(np.dstack(drop), axis=2))
        if drop
        else np.zeros((H, W), "float32")
    )
    # (b) Ratio change → |VH/VV diff|
    ratio01 = _pct_stretch(np.abs(arr.get(9, np.zeros((H, W), "float32"))))
    # (c) Variability → max(stdVV, stdVH)
    var_stack = (
        np.dstack([a for k, a in arr.items() if k in (10, 11)])
        if (10 in arr or 11 in arr)
        else None
    )
    var01 = (
        _pct_stretch(np.nanmax(var_stack, axis=2))
    ...
    risk = np.clip(
        (w_drop * drop01 + w_ratio * ratio01 + w_var * var01) / wsum, 0, 1
    ).astype("float32")
    ...
    if src_crs == dst_crs:
        ...
        dst_tr, dst_w, dst_h = calculate_default_transform(
            src_crs, dst_crs, W, H, *array_bounds(H, W, src_tr)
        )
    ...
    reproject(
        risk4326,
        risk_web,
        src_transform=dst_tr,
        src_crs=dst_crs,
        dst_transform=web_tr,
        dst_crs=dst_crs,
        dst_nodata=np.nan,
    ...
    risk_name = (
        f"risk_field_{field_id_for_name}_{risk_tag}.tif"
        if field_id_for_name
        else f"risk_{risk_tag}.tif"
    )
    ...
    try:
        ...
        poly = (
            shape(aoi_geojson["geometry"])
            if aoi_geojson.get("type") == "Feature"
            else shape(aoi_geojson)
        )
        aoi_mask_web = np.zeros_like(risk_u16, dtype="uint8")
        (south, west), (north, east) = (
            array_bounds(web_h, web_w, web_tr)[1::-1],
            array_bounds(web_h, web_w, web_tr)[3:1:-1],
        )
        ys = np.linspace(
            north - (north - south) / (2 * web_h),
            south + (north - south) / (2 * web_h),
            web_h,
        )
        xs = np.linspace(
            west + (east - west) / (2 * web_w),
            east - (east - west) / (2 * web_w),
            web_w,
        )
    ...
    meta = {
        ...
        "web_bounds": [
            [
                float(array_bounds(web_h, web_w, web_tr)[1]),
    ...
    # --- 5) Hotspots (use web grid for consistency with overlay)
    field_geom = None
    try:
        if aoi_geojson.get("type") == "Feature":
    ...
    hs = {}
    try:
        print(f"🔍 DEBUG: Starting hotspot extraction with tag: {tag}")
        print("🔍 DEBUG: Using web grid for hotspots to match overlay")
        print(
            f"🔍 DEBUG: risk_web.shape={risk_web.shape}, "
            f"risk_web.dtype={risk_web.dtype}"
        )
        rmin, rmax = np.nanmin(risk_web), np.nanmax(risk_web)
        print(f"🔍 DEBUG: risk_web min/max = {rmin:.4f}/{rmax:.4f}")
        print(f"🔍 DEBUG: field_geom type = {type(field_geom)}")
        print(f"🔍 DEBUG: web_tr = {web_tr}")
        print(f"🔍 DEBUG: dst_crs = {dst_crs}")
        geojson_path = str(HOT_DIR / f"hotspots_{tag}.geojson")
        print(f"🔍 DEBUG: OUT_GEOJSON = {geojson_path}")

        # Helper function to reproject component to web grid
        def reproject_to_web(component, name):
            """Reproject a component array to the web grid."""
            print(f"🔧 DEBUG: Reprojecting {name} to web grid")
            component_web = np.full((web_h, web_w), np.nan, "float32")

            if src_crs == dst_crs:
                component_4326 = component.copy()
            else:
                component_4326 = np.full((dst_h, dst_w), np.nan, "float32")
                reproject(
                    component,
                    component_4326,
                    src_transform=src_tr,
        ...
        # Reproject all components to web grid for consistency
        drop01_web = reproject_to_web(drop01, "drop01")
        ratio01_web = reproject_to_web(ratio01, "ratio01")
        var01_web = reproject_to_web(var01, "var01")

        hs = extract_hotspots(
            risk=risk_web,
            src_transform=web_tr,
            src_crs=dst_crs,
            field_polygon=field_geom,
            comps=[drop01_web, ratio01_web, var01_web],
            weights=[0.5, 0.3, 0.2],
            HOTSPOT_PERCENTILE=80,
            MIN_HOTSPOT_AREA_PIX=5,  # Reduced due to lower resolution
            MAX_HOTSPOTS=25,
            OUT_GEOJSON=geojson_path,
        )
        hs_keys = list(hs.keys())
        print(f"✅ DEBUG: Hotspot extraction succeeded. hs keys: {hs_keys}")
        if "geojson_path" in hs:
            print(f"✅ DEBUG: geojson_path = {hs['geojson_path']}")
        if "hotspots" in hs:
            hotspot_count = len(hs.get("hotspots", []))
            print(f"✅ DEBUG: Found {hotspot_count} hotspots")
    ...
    return {
        ...
        "probe_bin_url": f"/probe-bin/{probe_bin.name}",
