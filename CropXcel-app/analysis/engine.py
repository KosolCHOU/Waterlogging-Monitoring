...
# Make EE initialization optional for development/deployment
import os

ee_available = False

if os.getenv("DISABLE_EARTH_ENGINE", "").lower() == "true":
    print("[EE] Earth Engine disabled by environment variable")
    ee_available = False
elif os.getenv("USE_DEMO_MODE", "").lower() == "true":
    print("[EE] Running in demo mode - using sample data")
    ee_available = False
else:
    try:
        ...
        try:
            # Try authenticating first, then initialize
            print("[EE] Attempting to authenticate...")
            ee.Authenticate()
            ee.Initialize(project="angelic-edition-466705-r8")
            ee_available = True
            print("[EE] Successfully authenticated and initialized.")
        except Exception as auth_error:
            print(f"[EE] Earth Engine authentication failed: {auth_error}")
            print("[EE] Switching to demo mode with sample data.")
            ee_available = False
...
def export_stack_from_geom(
    geom_geojson: dict, out_tif: str, scale_m=10, orbit_pass=None, crs=None
) -> str:
    """
    ...
    """
    if not ee_available:
        # Demo mode: create a sample analysis result
        print("[EE] Running in demo mode - generating sample data")
        import numpy as np
        from pathlib import Path

        # Create a small demo raster with sample waterlogging data
        out_path = Path(out_tif)
        out_path.parent.mkdir(parents=True, exist_ok=True)

        # Generate sample data (50x50 pixels)
        height, width = 50, 50

        # Create realistic S1 data with proper band structure
        # Generate base values
        vv_base = 0.05 + np.random.rand(height, width) * 0.1  # Linear power
        vh_base = 0.01 + np.random.rand(height, width) * 0.02

        # Generate current values (with some flooding events)
        flood_mask = np.random.rand(height, width) > 0.85  # 15% flooded
        vv_curr = vv_base.copy()
        vh_curr = vh_base.copy()

        # Simulate flooding (lower backscatter)
        vv_curr[flood_mask] *= 0.3  # Significant drop
        vh_curr[flood_mask] *= 0.4

        # Calculate ratios and differences
        vh_vv_curr = vh_curr / vv_curr
        vh_vv_base = vh_base / vv_base
        vh_vv_diff = vh_vv_curr - vh_vv_base

        # Log ratios (dB)
        vv_logratio_db = 10 * np.log10(vv_curr / vv_base)
        vh_logratio_db = 10 * np.log10(vh_curr / vh_base)

        # Standard deviations (simulate temporal variability)
        vv_std = 0.5 + np.random.rand(height, width) * 1.0
        vh_std = 0.5 + np.random.rand(height, width) * 1.0

        # Stack bands in expected order:
        # 1=S1_VV_CURR, 2=S1_VH_CURR, 3=S1_VH_VV_CURR, 4=S1_VV_BASE, 5=S1_VH_BASE,
        # 6=S1_VH_VV_BASE, 7=S1_VV_LOGRATIO_DB, 8=S1_VH_LOGRATIO_DB, 9=S1_VH_VV_DIFF,
        ...
        try:
            ...
            # Extract bounds from actual field geometry
            if geom_geojson.get("type") == "Polygon":
                ...
                # Fallback bounds for Cambodia if geometry parsing fails
                bounds = [104.0, 12.0, 104.1, 12.1]
            ...
            with rasterio.open(
                ...
                for i in range(bands):
                    dst.write(sample_data[i], i + 1)

            print(f"[EE] Demo analysis saved to: {out_tif}")
            return out_tif

        except ImportError:
            raise Exception(
                "Demo mode requires rasterio library for sample data generation"
            )
    ...
    base_core = base_core.addBands(
        base_core_lin.select("VH_VV").rename("S1_VH_VV_BASE")
    )

    # Ratios & STD
    vv_logratio_db = lin_to_db_safe(
        evt_vv.divide(base_core_lin.select("VV_lin"))
    ).rename("S1_VV_LOGRATIO_DB")
    vh_logratio_db = lin_to_db_safe(
        evt_vh.divide(base_core_lin.select("VH_lin"))
    ).rename("S1_VH_LOGRATIO_DB")
    ...
    geemap.ee_export_image(
        stack, out_tif, scale=scale_m, region=geom, file_per_band=False, crs=crs
    )
...
def export_s1_timeseries(
    ...
    """
    if not ee_available:
        # Demo mode: create a sample time series CSV
        print("[EE] Running in demo mode - generating sample timeseries data")
        import pandas as pd
        from pathlib import Path
        import numpy as np
        ...
        # Generate sample data with realistic waterlogging patterns
        data = []
        for date in dates:
            # Simulate seasonal variation and random events in dB domain
            base_vv_db = -12.0 + np.random.normal(0, 1.5)
            base_vh_db = -18.0 + np.random.normal(0, 2.0)

            # Simulate occasional flooding events (lower backscatter)
            if np.random.rand() > 0.85:  # 15% chance of flooding
                event_vv_db = base_vv_db - np.random.uniform(2, 5)
                event_vh_db = base_vh_db - np.random.uniform(1, 3)
            else:
                event_vv_db = base_vv_db + np.random.normal(0, 0.5)
                event_vh_db = base_vh_db + np.random.normal(0, 0.8)

            # Convert to linear domain for S1_VV_CURR and S1_VH_CURR
            event_vv_linear = 10 ** (event_vv_db / 10)
            event_vh_linear = 10 ** (event_vh_db / 10)
            base_vv_linear = 10 ** (base_vv_db / 10)
            base_vh_linear = 10 ** (base_vh_db / 10)

            data.append(
                {
                    "date": date.strftime("%Y-%m-%d"),
                    "S1_VV_CURR": event_vv_linear,
                    "S1_VH_CURR": event_vh_linear,
                    "S1_VV_LOGRATIO_DB": event_vv_db - base_vv_db,
                    "S1_VH_LOGRATIO_DB": event_vh_db - base_vh_db,
                    "S1_VH_VV_CURR": event_vh_linear / event_vv_linear,
                    "S1_VH_VV_DIFF": (event_vh_linear / event_vv_linear)
                    - (base_vh_linear / base_vv_linear),
                    "S1_VH_VV_BASE": base_vh_linear / base_vv_linear,
                    "S1_VH_STD": np.random.uniform(0.5, 2.0),
                    "image_count": np.random.randint(1, 8),
                }
            )
        ...
        print(f"[EE] Demo timeseries saved to: {out_csv}")
        return out_csv
    ...
    def _feature_for_step(step_end):
        ...
        ic_evt_raw = load_s1_ic(
            evt_start, step_end.advance(1, "day"), geom, orbit_pass
        ).map(db_to_linear_keep)
        ic_base_raw = load_s1_ic(
            base_start, base_end.advance(1, "day"), geom, orbit_pass
        ).map(db_to_linear_keep)
        ...
        vv_logratio_db = lin_to_db_safe(vv_curr.divide(vv_base)).rename(
            "S1_VV_LOGRATIO_DB"
        )
        vh_logratio_db = lin_to_db_safe(vh_curr.divide(vh_base)).rename(
            "S1_VH_LOGRATIO_DB"
        )
